...
# -----------------------------------------------
# 1. Apply Gaussian Blur separately to R, G, B
# -----------------------------------------------
...
def fitness(sigmas, noisy_img, original_img):
    ...
    return -mse  # PSO maximizes fitness (we want to minimize MSE)
...
# -----------------------------------------------
# 3. Main Code: Load Coffee Image, Add Noise, Run PSO
# -----------------------------------------------

# Load new image (coffee) and normalize
original_img = img_as_float(data.coffee())
original_img = cv2.resize(original_img, (128, 128))  # Resize for speed
...
# Define bounds for sigma values (R, G, B)
